...
- Познакомиться с интересным живым open-source проектом

## Подготовка
- Завести dev.to локально
- Настроить свой Skylight для мониторинга локального dev.to
- Настроить свой NewRelic для мониторинга локального dev.to
- Настроить свой Scout для мониторинга локального dev.to
...
- Нагрузите главную страницу запросами, так чтобы они отразились в системах мониторинга
- Сделайте `benchmark` с помощью `ab`
- Сделайте оптимизацию
- Снова нагрузите страницу запросами и перезапустите `benchmark`

Если вы посчитаете, что применить кэширование здесь целесообразно, оформите обоснованный `PR` с этим предложением. Напишите в описании `PR`, какая была гипотеза, откуда она взялась, как вы проверяли гипотезу, какие результаты получили.

## Bonus №1
Обратите внимание на класс `FlareTag`. Он фигурирует в отчётах систем мониторинга как часто вызываемый код приложения. Можно ли оптимизировать что-то в этом классе, или в методах, которые он использует?

Если вы придумаете как его оптимизировать, попробуйте сделать обоснованный `PR`, как в основной части задания.

## Bonus №2
Добейтесь запуска приложения в режиме `RAILS_ENV=production`.

Можно просто начать, и далее устранять возникающие проблемы одну за одной.

Если получится запустить приложение в `production` режиме, сравните метрики с режимом `development`. Сравните отчёты `Rack-mini-profiler` с `development`.

Сделайте возможность удобно запустить `local-production`, добавьте информацию об этой возможности в `Readme`.
